...
> => {
  try {
    ...
    const lastMonthUsage =
      lastMonthResponse.data?.reduce(
        (sum, req) => sum + (req.requestAmount || 0),
        0,
      ) || 0;
    const currentMonthUsage =
      currentMonthResponse.data?.reduce(
        (sum, req) => sum + (req.requestAmount || 0),
        0,
      ) || 0;
    ...
    const totalUsedFromRequests =
      allApprovedResponse.data?.reduce(
        (sum, req) => sum + (req.requestAmount || 0),
        0,
      ) || 0;
    ...
    console.log(
      ...
      allApprovedResponse.data?.length || 0,
    );
    console.log(
      "Fechas de muestra:",
      allApprovedResponse.data?.slice(0, 3).map((r) => ({
        fecha: r.requestDate,
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const monthData =
        allApprovedResponse.data?.filter((req) => {
      ...
      const monthAmount = monthData.reduce(
        (sum, req) => sum + (req.requestAmount || 0),
        0,
      );

      console.log(
        `${monthNames[monthDate.getMonth()]} ${monthDate.getFullYear()}: Q${monthAmount} (${monthData.length} solicitudes)`,
      );
 ...
 */
export const getPendingSolicitudesStats = async (): Promise<
  ApiResponse<PendingSolicitudesStats[]>
> => {
  try {
    ...
    console.error(
      "Error al obtener estadísticas de solicitudes pendientes:",
      error,
    );
    return {
      success: false,
