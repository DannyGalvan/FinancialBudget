...
{
    public class UpdateBudgetItemValidator : UpdateValidator<BudgetItemRequest, long?>
    {
        public UpdateBudgetItemValidator()
        {
            RuleFor(x => x.Id)
                .NotEmpty().WithMessage("El Id del item del presupuesto es requerido")
                .Must(Util.HasValidId).WithMessage("El Id del item del presupuesto no es valido");
            RuleFor(x => x.Amount)
